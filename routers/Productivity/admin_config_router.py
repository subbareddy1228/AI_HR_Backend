...
def create_entity(payload: ProductiveEntityCreate, db: Session = Depends(get_db), _=Depends(require_roles(["superadmin", "admin", "company"]))):
    try:
...
def get_entities(db: Session = Depends(get_db), _=Depends(require_roles(["superadmin", "admin", "company"]))):
    return list_entities(db)

@router.get("/entities/{entity_id}", response_model=ProductiveEntityResponse)
def get_single(entity_id: int, db: Session = Depends(get_db), _=Depends(require_roles(["superadmin", "admin", "company"]))):
...
def patch_entity(entity_id: int, payload: ProductiveEntityCreate, db: Session = Depends(get_db), _=Depends(require_roles(["superadmin", "admin", "company"]))):
    try:
...
def remove_entity(entity_id: int, db: Session = Depends(get_db), _=Depends(require_roles(["superadmin", "admin", "company"]))):
    try:
