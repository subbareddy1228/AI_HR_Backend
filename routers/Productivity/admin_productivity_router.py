...
def overview(db: Session = Depends(get_db), _=Depends(require_roles(["superadmin", "admin", "company"]))):
    return get_org_summary(db)

@router.get("/team/{team_id}")
def team_summary(team_id: int, db: Session = Depends(get_db), _=Depends(require_roles(["superadmin", "admin", "company"]))):
    return get_team_summary(db, team_id)

@router.get("/department/{department_id}")
def department_summary(department_id: int, db: Session = Depends(get_db), _=Depends(require_roles(["superadmin", "admin", "company"]))):
