...
def _is_duplicate(
    db: Session, employee_id: str, punch_time: datetime, punch_type: PunchTypeEnum, window_minutes: int
) -> bool:
...
def _is_ip_whitelisted(db: Session, ip: str) -> bool:
    return db.query(WhitelistedIP).filter_by(ip_address=ip, is_active=True).first() is not None


def _find_geofence(db: Session, lat: float, lng: float) -> tuple[Optional[GeoLocation], bool]:
...
class PunchService:
    ...
    def biometric_punch(
        ...
        if _is_duplicate(db, employee_id, punch_time, punch_type, settings.duplicate_punch_window_minutes):
...
class AttendanceAggregationService:
    ...
    @staticmethod
    def upsert_daily_record(db: Session, employee_id: str, record_date: date) -> Optional[AttendanceRecord]:
        settings = _get_settings(db)
        punches = (
        ...
        if not punches:
            db.query(AttendanceRecord).filter_by(employee_id=employee_id, date=record_date).delete()
            db.commit()
            return None
        ...
        overtime_hours = max(Decimal("0"), effective_hours - Decimal(str(settings.min_work_hours)))

        # Late arrival
        is_late, late_minutes = False, 0
        if first_in:
            sched_start = datetime.combine(record_date, settings.work_start_time).replace(tzinfo=timezone.utc)
        ...
        if last_out:
            sched_end = datetime.combine(record_date, settings.work_end_time).replace(tzinfo=timezone.utc)
        ...
        # Majority capture method
        methods = [p.capture_method for p in punches]
        capture_method = max(set(methods), key=methods.count) if methods else ""

        # Upsert
        record = db.query(AttendanceRecord).filter_by(employee_id=employee_id, date=record_date).first()
        if record is None:
...
class DeviceSyncService:
    ...
    def sync_device(
        ...
        try:
            ...
            for raw in raw_records:
                ...
                if not _is_duplicate(db, emp_id, punch_time, punch_type, settings.duplicate_punch_window_minutes):
                    ...
                    AttendanceAggregationService.upsert_daily_record(db, emp_id, punch_time.date())
                    count += 1
    ...
    def _fetch_from_device(device: BiometricDevice) -> list[dict]:
        """
        STUB – replace with actual SDK call for each vendor.
        ...
        eSSL (iclock REST API):
        ────────────────────────
            import requests
            url  = f"http://{device.ip_address}/iclock/data/iclock/push/"
            resp = requests.get(url, timeout=5)
            rows = resp.json().get("data", [])
            return [{"employee_id": r["pin"], "punch_time": datetime.fromisoformat(r["time"]),
                     "punch_type": "check_in"} for r in rows]
        """
        logger.info("STUB: _fetch_from_device called for %s %s", device.vendor, device.ip_address)
        return []

    @staticmethod
    def sync_all_online(db: Session, initiated_by: Optional[int] = None) -> list[DeviceSyncLog]:
        devices = db.query(BiometricDevice).filter_by(status=DeviceStatusEnum.Online, is_active=True).all()
    ...
    def reconnect_offline(db: Session) -> list[BiometricDevice]:
        reconnected = []
        offline = db.query(BiometricDevice).filter_by(status=DeviceStatusEnum.Offline, is_active=True).all()
        for device in offline:
            try:
                sock = socket.create_connection((device.ip_address, device.sdk_port), timeout=3)
                sock.close()
                device.status = DeviceStatusEnum.Online
...
class OfflineSyncService:
    ...
    def flush_queue(db: Session) -> dict:
        pending  = db.query(OfflinePunchQueue).filter_by(status=OfflineStatusEnum.pending).order_by("punch_time").all()
        synced, failed = 0, 0
        settings = _get_settings(db)
        ...
        for item in pending:
            try:
                if not _is_duplicate(db, item.employee_id, item.punch_time,
                    ...
                    AttendanceAggregationService.upsert_daily_record(db, item.employee_id, item.punch_time.date())
...
class DashboardService:
    ...
    def today_stats(db: Session) -> dict:
        ...
        try:
            from models.employee import Employee  # import from existing employee model
            total_employees = db.query(Employee).filter_by(status="Active").count()
        except Exception:
            total_employees = 0

        present    = records.filter(AttendanceRecord.status.in_(["present", "late"])).count()
        ...
        overtime   = db.query(func.sum(AttendanceRecord.overtime_hours)).filter_by(date=today).scalar() or Decimal("0")
    ...
    def device_report(db: Session, device: BiometricDevice) -> dict:
        return {
            ...
            "total_punches":  db.query(AttendancePunch).filter_by(biometric_device_id=device.id).count(),
            "check_ins":      db.query(AttendancePunch).filter_by(biometric_device_id=device.id, punch_type=PunchTypeEnum.check_in).count(),
            "check_outs":     db.query(AttendancePunch).filter_by(biometric_device_id=device.id, punch_type=PunchTypeEnum.check_out).count(),
